...
namespace 云湖WP
{
    ...
    public sealed partial class GroupsPage : Page
    {
        ...
        private ObservableCollection<FriendContactItem> _groupsList = new ObservableCollection<FriendContactItem>();
        ...
        protected override async void OnNavigatedTo(NavigationEventArgs e)
        {
            ...
            GroupsListView.ItemsSource = _groupsList;
        ...
        private async Task LoadGroupsAsync()
        {
            ...
            try
            {
                ...
                if (result.IsSuccess && result.Groups != null)
                {
                    _groupsList.Clear();
                    foreach (var item in result.Groups)
                    {
                        _groupsList.Add(item);
                    ...
                    if (_groupsList.Count == 0 && EmptyPanel != null)
                    {
                        EmptyPanel.Visibility = Visibility.Visible;
                    }

                    PreloadAvatars(result.Groups);
                }
                else
                {
                    if (_groupsList.Count == 0 && EmptyPanel != null)
            ...
            if (errMsg != null)
            {
                var dialog = new MessageDialog(errMsg, "云湖群聊");
